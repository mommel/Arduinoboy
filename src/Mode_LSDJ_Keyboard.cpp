...
void modeLSDJKeyboardSetup() {
  ...
  /* The stuff below makes sure the code is in the same state as LSDJ on reset /
   * restart, mode switched, etc. */

  for (int rst = 0; rst < 5; rst++)
    sendKeyboardByteToGameboy(keyboardOctDn);  // Return lsdj to the first
                                               // octave
  for (int rst = 0; rst < 41; rst++)
    sendKeyboardByteToGameboy(
        keyboardInsDn);  // Return lsdj to the first instrument
    ...
    if (serial->available()) {            // If MIDI is sending
      ...
      if (incomingMidiByte & 0x80) {
        ...
        */
        ...
        // Weee hello world bitwise and. ... make the second hex digit zero so
        // we can have a simple case statement
        // - the second digit is usually the midi channel 0 to F (1-16) unless
        // its a 0xF0 message...
        switch (incomingMidiByte & 0xF0) {
          case 0x90:
            // Note-On Status Message (Note: we have to treat this carefully
            // because note status isnt sent on every note-on, damn it) There
            // are 3 bytes total we need: Channel, Note, and velocity, these wil
            // be assigned to a array until we have the velocity, at that point
            // we can then call our note out function to LSDJ
            midiNoteOnMode = true;  // Set our stupid "Note on mode" on
            midiData[0] =
                incomingMidiByte;  // Assign the byte to the first position of a
                                   // data array. (this is the midi channel)
            midiData[1] = false;   // Force the second position to false (this
                                   // will hold the note number)
            break;
          case 0xC0:
            // Program change message
            midiProgramChange = true;  // Set our silly "Program Change mode"
                                       // ... we need to get the next byte later
            midiNoteOnMode = false;    // Turn Note-on mode off
            midiData[0] =
                incomingMidiByte -
                48;  // Set the number to a "note on" message so we can use the
                     // same "channel" variable as note on messages
            break;
          case 0xF0:
        ...
        // It wasnt a status bit, so lets assume it was a note message if the
        // last status message was note-on.
        if (!midiData[1]) {
          // If we dont have a note number, we assume this byte is the note
          // number, get it...
          ...
          midiData[1] =
              false;  // Set the note to false, forcing to capture the next note
        }
...
void changeLSDJInstrument(byte channel, byte message) {
  ...
  if (channel == (0x90 + memory[MEM_KEYBD_CH]) &&
      keyboardCurrentIns != keyboardLastIns) {
    // if its on our midi channel and the instrument isnt the same as our
    // currrent
    if (!memory[MEM_KEYBD_COMPAT_MODE]) {
      sendKeyboardByteToGameboy(
          0x80 | message);  // <- this is suppose to work but doesn't :/
    } else {
      // We will find out which is greater, the current instrument or the last
      // instrument. then cycle up or down to that instrument
      if (keyboardCurrentIns > keyboardLastIns) {
        ...
        for (keyboardCount = 0; keyboardCount < keyboardDiff; keyboardCount++) {
          sendKeyboardByteToGameboy(
              keyboardInsUp);  // send the instrument up command to go up to the
        ...
        for (keyboardCount = 0; keyboardCount < keyboardDiff; keyboardCount++) {
          sendKeyboardByteToGameboy(
              keyboardInsDn);  // send the instrument down command to down to
                               // the requested instrument
        }
      }
    }
    keyboardLastIns = keyboardCurrentIns;  // assign our instrument for future
...
void playLSDJNote(byte channel, byte note, byte velocity) {
  if (channel == (0x90 + memory[MEM_KEYBD_CH]) &&
      velocity >
          0x00) {  // If midi channel = ours and the velocity is greater then 0
    if (note >= keyboardNoteStart) {
      keyboardNoteOffset = 0;
      note = note -
             keyboardNoteStart;  // subtract the octave offset to get a
                                 // value ranging from 0 to 48 for comparison

      keyboardCurrentOct = note / 0x0C;  // get a octave value from 0 to 4 by
                                         // deviding the current note by 12
      changeLSDJOctave();

      if (note >= 0x3C)
        keyboardNoteOffset = 0x0C;  // if the note really high we need to use
                                    // the second row of keyboard keys
      note = (note % 12) + keyboardNoteOffset;  // get a 0 to 11 range of notes
                                                // and add the offset
      sendKeyboardByteToGameboy(
          keyboardNotes[note]);  // and finally send the note

    } else if (note >=
               keyboardStartOctave) {  // If we are at the octave below notes
      keyboardDiff = note - keyboardStartOctave;  // Get a value between 0 and
                                                  // 11
      if (keyboardDiff < 8 && keyboardDiff > 3)
        sendKeyboardByteToGameboy(
            0xE0);  // if we are sending cursor values we have to send a 0xE0
                    // byte for "extended" pc keyboard mode
      sendKeyboardByteToGameboy(
          keyboardCommands
              [note - keyboardStartOctave]);  // send the byte corrisponding to
                                              // the note number in the keyboard
                                              // command array
    }
  }
}
...
void changeLSDJOctave() {
  if (keyboardCurrentOct != keyboardLastOct) {
    if (!memory[MEM_KEYBD_COMPAT_MODE]) {  // This new mode doesnt work yet. :/
      ...
      /// We will find out which is greater, the current octave or the last.
      /// then
...
void sendKeyboardByteToGameboy(byte send_byte) {
  ...
  for (countLSDJTicks = 0; countLSDJTicks < 8;
       countLSDJTicks++) {  // we are going to send 8 bits, so do a loop 8 times
    ...
    send_byte >>=
        1;  // bitshift right once for the next bit we are going to send
  }
  ...
  while (usbMIDI.read(memory[MEM_KEYBD_CH] + 1)) {
    switch (usbMIDI.getType()) {
      case 0x80:  // note off
        ...
        break;
      case 0x90:  // note on
        playLSDJNote(0x90 + memory[MEM_KEYBD_CH], usbMIDI.getData1(),
                     usbMIDI.getData2());
        break;
      case 0xC0:  // PG
