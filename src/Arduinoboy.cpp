...
byte defaultMemoryMap[MEM_MAX] = {
    ...
    15,  // keyboardInstrumentMidiChannel - midi channel for keyboard
         // instruments in lsdj. (0-15 = 1-16)
    1,   // Keyboard Compatability Mode
    1,   // Set to true if you want to have midi channel set the instrument
         // number / doesnt do anything anymore

    0, 1, 2, 3,  // midiOutNoteMessageChannels - midi channels for lsdj midi
                 // out note messages Default: channels 1,2,3,4
    0, 1, 2, 3,  // midiOutCCMessageChannels - midi channels for lsdj midi out
                 // CC messages Default: channels 1,2,3,4
    1, 1, 1,
    ...
    1, 1, 1,
    1,  // midiOutCCScaling - CC Scaling- Setting to 1 scales the CC value
        // range to 0-127 as oppose to lsdj's incomming 00-6F (0-112) or 0-F
        // (0-15)
    1, 2, 3, 7, 10, 11,
    12,  // pu1: midiOutCCMessageNumbers - CC numbers for lsdj midi out, if
         // CCMode is 1, all 7 ccs are used per channel at the cost of a
         // limited resolution of 0-F
    ...
    0,      // sync map midi channel start (0-15 = 1-16) (for song rows 0x80 to
            // 0xFF it's this channel plus 1)
...
#include "pinassignments/teensy.h"
#endif

#ifdef BUILD_ARM32U4
#include <MIDIUSB.h>

#include "pinassignments/atmega32u4.h"
#endif
...
#include "pinassignments/sam.h"
#endif

#ifdef BUILD_AVR_OTHER
#include <PS2Keyboard.h>

#include "pinassignments/avr.h"
#endif
...
#include "pinassignments/esp32.h"
#endif

#if not defined(BUILD_AVR_OTHER) && not defined(BUILD_SAM) &&   \
    not defined(BUILD_ARM32U4) && not defined(BUILD_TEEENSY) && \
    not defined(BUILD_ESP32)
platform = undefined
#endif
...
#include <EEPROM.h>
    boolean alwaysUseDefaultSettings =
        false;  // set to true to always use the settings below, else they are
                // pulled from memory for the software editor
#else
boolean alwaysUseDefaultSettings =
    true;  // set to true to always use the default settings, in Due board is
...
uint8_t mapQueueWaitUsb = 5;     // 5ms - Needs to be longer because message
                                 // packet is processed all at once
