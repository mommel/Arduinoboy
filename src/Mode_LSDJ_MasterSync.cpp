...
void modeLSDJMasterSync() {
  while (1) {
    ...
    if (readgbClockLine) {                          // If Gb's Clock is On
      ...
      countClockPause =
          0;  // Reset our wait timer for detecting a sequencer stop

      readGbSerialIn = readGbSerialIn
                       << 1;  // left shift the serial byte by one to append new
                              // bit from last loop
      readGbSerialIn =
...
void sendMidiClockSlaveFromLSDJ() {
  if (!countGbClockTicks) {   // If we hit 8 bits
    if (!sequencerStarted) {  // If the sequencer hasnt started
      serial->write(
          (0x90 +
           memory[MEM_LSDJMASTER_MIDI_CH]));  // Send the midi channel byte
