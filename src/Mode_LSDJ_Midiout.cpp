...
#ifdef USE_TEENSY
    usbMIDI.sendNoteOff(midioutNoteHold[m][x], 0,
                        memory[MEM_MIDIOUT_NOTE_CH + m] + 1);
#endif
#ifdef USE_LEONARDO
    midiEventPacket_t packet = {0x08, 0x80 | memory[MEM_MIDIOUT_NOTE_CH + m],
                                midioutNoteHold[m][x], 0};
    MidiUSB.sendMIDI(packet);
    MidiUSB.flush();
#endif
  }
...
#endif
#ifdef USE_LEONARDO
  midiEventPacket_t packet = {0x09, 0x90 | memory[MEM_MIDIOUT_NOTE_CH + m], n,
                              127};
  MidiUSB.sendMIDI(packet);
  MidiUSB.flush();
#endif
...
void playCC(byte m, byte n) {
  ...
  if (memory[MEM_MIDIOUT_CC_MODE + m]) {
    ...
    serial->write(midiData, 3);
#ifdef USE_TEENSY
    usbMIDI.sendControlChange((memory[MEM_MIDIOUT_CC_NUMBERS + n]), v,
                              memory[MEM_MIDIOUT_NOTE_CH + m] + 1);
#endif
#ifdef USE_LEONARDO
    midiEventPacket_t packet = {0x0B,
                                0xB0 | (memory[MEM_MIDIOUT_NOTE_CH + m] + 1),
                                (memory[MEM_MIDIOUT_CC_NUMBERS + n]), v};
    MidiUSB.sendMIDI(packet);
    MidiUSB.flush();
#endif
  } else {
...
#endif
#ifdef USE_LEONARDO
  midiEventPacket_t packet = {0x0C,
                              0xC0 | (memory[MEM_MIDIOUT_NOTE_CH + m] + 1), n};
  MidiUSB.sendMIDI(packet);
  MidiUSB.flush();
#endif
}
...
#endif
#ifdef USE_LEONARDO
    midiEventPacket_t packet = {0x0B, 0xB0 | memory[MEM_MIDIOUT_NOTE_CH + m],
                                123, 127};
    MidiUSB.sendMIDI(packet);
    MidiUSB.flush();
#endif
