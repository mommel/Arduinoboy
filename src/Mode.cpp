...
void setMode() {
  ...
  if (!memory[MEM_FORCE_MODE] && buttonDepressed) {  // if the button is pressed
    ...
    if (memory[MEM_MODE] > (NUMBER_OF_MODES - 1))
      memory[MEM_MODE] =
          0;  // if the mode is greater then 4 it will wrap back to 0
#ifndef USE_DUE
    if (!memory[MEM_FORCE_MODE])
      EEPROM.write(MEM_MODE,
                   memory[MEM_MODE]);  // write mode to eeprom if we arnt
                                       // forcing a mode in the config
#endif
...
void sequencerStart() {
  ...
  countSyncTime = 0;   // Used to count a custom amount of clock ticks (2/4/8)
                       // for sync effects
